...
export default function MessagesPage() {
  ...
  async function openConversation(conv: Conversation) {
    ...
    if (token) {
      try {
        const ws = new WebSocket(`${SOCKET_URL.replace('http', 'ws')}/ws/conversations/${conv.id}?token=${token}`);
        ws.onmessage = (e) => {
        ...
        <div className="flex-1 overflow-y-auto">
          {loadingConvs ? (
            ...
            conversations.map(conv => {
              ...
              const otherUserId = user.role === 'buyer' ? conv.agent_id : conv.buyer_id;
              return (
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-center mb-0.5">
                      <p className="font-semibold text-sm text-gray-900 truncate">
                        {user.role === 'buyer' ? `Agent ${conv.agent_id.slice(0, 6)}` : `Buyer ${conv.buyer_id.slice(0, 6)}`}
                      </p>
        ...
        <div className={cn('flex-1 flex flex-col bg-white', showList && 'hidden md:flex')}>
          <div className="flex items-center gap-3 px-4 py-3 border-b border-gray-100 bg-white sticky top-0 z-10">
            ...
            <div className="w-10 h-10 rounded-full bg-blue-600 flex items-center justify-center shrink-0 text-white font-bold text-sm">
              {(user.role === 'buyer' ? active.agent_id : active.buyer_id).slice(0, 2).toUpperCase()}
            </div>
            <div className="flex-1 min-w-0">
              <p className="font-semibold text-gray-900 text-sm">
                {user.role === 'buyer' ? 'Agent' : 'Buyer'}
              </p>
              {active.property_id && (
